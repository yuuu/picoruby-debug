...
- **`tools/mrdebug/ui/local_console.rb`** (host builds only) —
  `MRDebug::UI::LocalConsole`: the `(prdb)` prompt, one `STDIN.gets` per
  command. Reading exactly one line at a time is what avoids the old
  `picoruby-editor`-based design's known bug (piped/pasted multi-command
  input losing everything after a resuming command) — there's no shared
  read-ahead buffer to lose data from.
